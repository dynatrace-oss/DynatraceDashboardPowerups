    ...
    decodeEntry(key, value) {
        switch (key) {
            case "cp" /* Capture */:
                ...
                this.captureMode = value === '1' ? api_1.CaptureMode.On : api_1.CaptureMode.Off;
                break;
            case "cr" /* CaptureCrashes */:
                // 1 (always on) and 2 (only on WiFi) are treated the same
                this.captureCrashes = value === '1' ? api_1.CaptureMode.On : api_1.CaptureMode.Off;
                break;
            case "er" /* CaptureErrors */:
                // 1 (always on) and 2 (only on WiFi) are treated the same
                this.captureErrors = value === '1' ? api_1.CaptureMode.On : api_1.CaptureMode.Off;
                break;
