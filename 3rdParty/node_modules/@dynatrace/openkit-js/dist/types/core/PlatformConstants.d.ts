/**
 * The OpenKit-JavaScript version.
 * For now, it must be equal to the mobile agent version.
 */
export declare const openKitVersion = "7.0.0000";
/**
 * Constant for the OneAgent Mobile Communication Protocol Version 3.
 */
export declare const protocolVersion = 3;
/**
 * Constant for all OpenKit platforms.
 */
export declare const platformTypeOpenKit = 1;
/**
 * The OpenKit-JavaScript agent technology type constant.
 */
export declare const agentTechnologyType = "okjs";
export declare const defaultServerId = 1;
//# sourceMappingURL=PlatformConstants.d.ts.map